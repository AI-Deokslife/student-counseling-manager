...
- 일정 수정/취소
- 상담 완료 처리
- 교사가 허용한 기기에 한해 상담 1시간 전 푸시 알림

푸시 알림에는 학생 이름, 상담 내용, 메모를 포함하지 않고 `1시간 후 상담 일정이 있습니다.`라는 최소 정보만 표시한다. 알림은 교사가 기기별로 직접 허용하거나 해제할 수 있다.
...
`workers.dev` 초기 배포처럼 Access 정책을 아직 연결하지 못한 환경에서는 Worker Secret 기반 관리자 로그인을 보조 인증 계층으로 사용할 수 있다. DB 관리자 비밀번호는 8자 이상으로 설정한다. 초기 비밀번호 원문은 Worker Secret 설정 절차로만 전달하며, 코드나 D1에는 저장하지 않는다. bcrypt cost 12 해시만 Worker Secret 또는 `admin_credentials`에 저장한다. 관리자가 앱에서 비밀번호를 변경하면 해시와 변경 시각만 갱신하며, 변경 즉시 기존 관리자 세션은 무효화한다. 로그인 세션은 짧은 만료시간의 `HttpOnly`, `Secure`, `SameSite=Strict` 쿠키를 사용한다.
...
Local Edition의 PIN은 강력한 보안경계가 아니라 우발적 열람 방지 기능으로 정의한다. 로컬 모드 진입 시 관리자 비밀번호를 요구하고, 초기 비밀번호는 `12345678`이며 로그인 후 설정에서 변경할 수 있다. 비밀번호는 최소 8자 이상으로 설정한다. 같은 탭에서 새로고침해도 로그인 상태는 유지하고, 로컬 로그아웃 또는 탭 종료 후에는 다시 잠금 상태가 된다. 비밀번호 원문은 IndexedDB에 저장하지 않는다.
